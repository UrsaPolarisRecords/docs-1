...
return ( <div className="mb-6"> <NextLink href={`/${locale}/supported-networks`} className="text-blue-500 hover:underline"> {t('supportedNetworks.back')} </NextLink> </div> ) }
...
<h1 className="text-heading-large text-white xs:text-heading-xlarge">{props.network.fullName}</h1>
