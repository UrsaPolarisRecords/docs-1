...
Currently, the Token API supports Ethereum, Binance Smart Chain (BSC), Polygon, Optimism, Base, Unichain, Avalanche, and Arbitrum One.

### Does the Token API support NFTs?

Yes, The Graph Token API currently supports NFT with the ERC-721 and ERC-1155 NFTs, with support for additional standards planned. Endpoints are offered for ownership, collection stats, metadata, sales, holders, and transfer activity.
...
The Token API currently tracks prices on Uniswap v2, Uniswap v3, and Uniswap v4, with plans to support additional exchanges in the future.
